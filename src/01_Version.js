...
! DECISION DE PRODUCTO, CORREGIDA POR FRANCO EL MISMO DIA, TEXTUAL: "Solo lo manual. Lo proyectado no." Se trae UNICAMENTE el origen 'guardado' -- lo que ya salio de estas mismas columnas K/O/S via aplicarGuardarProyeccion. NO recurrentes, NO presupuesto base, NO proyecciones sueltas del shell, NO origen no reconocido. La primera version de esta misma tarde habia elegido sumar los CINCO origenes que DEVTOOL_ProyeccionAbm.js distingue, leyendo el pedido como "traer todo lo proyectado"; Franco corrigio la lectura: plasmar solo el guardado NO es circular, es RESTAURAR y COPIAR HACIA ADELANTE (recuperar la hoja tras limpiarla, o arrancar un mes desde lo presupuestado el anterior y editar la diferencia), y "Monto a Proyectar" (K/O/S) es la superficie de trabajo MANUAL de Franco -- volcar ahi lo que el sistema infirio borraria la linea entre lo decidido y lo deducido.
! LA CORRECCION ELIMINA UN MODO DE FALLA, no solo simplifica: la version de los cinco origenes necesitaba un aviso de "riesgo de doble conteo" porque "Guardar Proyeccion" nunca retira shell/recurrentes/otros de la BD, y volver a guardar un mes ya plasmado los hubiera contado dos veces en el Tablero. Con solo 'guardado', "Guardar Proyeccion" retira EXACTAMENTE esas filas al re-guardar el mismo mes (su propia decision 4) -- la clase de bug desaparece en vez de mitigarse. _avisoDobleConteoPp y su llamador se retiran enteros del modulo.
! MONEDA, SIN CONVERSION SILENCIOSA (pedido explicito, sigue vigente con un solo origen): K/O/S es una sola moneda por hoja (J4). Si una cuenta mezcla monedas para el periodo -- por ejemplo, dos guardados del mismo mes con J4 en monedas distintas -- o esta proyectada en una moneda distinta de J4, esa celda NO se escribe -- se cuenta y se reporta como anomalia, nunca se inventa una tasa de conversion.
...
+ devtools/probar_presupuesto_plasmar.js (nuevo, reescrito tras la correccion): mutaciones dirigidas -- suma dentro de 'guardado' por cuenta, celdas ya cargadas (pisa con conteo exacto), cuenta con dos guardados en monedas distintas (mezcla, no escribe), moneda distinta de la del presupuesto (no escribe), cuenta que ya no existe en el Plan vivo (no escribe), filas de shell/recurrentes/base/otros presentes en la BD del mes que NO entran al plan ni a los totales, mes vacio (nada que hacer, sin dialogo), confirmacion solo cuando hay pisa, verificacion por relectura con reversion de lote ante una escritura que no verifica, y el deshacer protegiendo una edicion manual posterior.
