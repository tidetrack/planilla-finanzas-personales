...
> # ATENCION: ESTE MAPA ES PRE-SWAP (anterior al 2026-08-18)
>
> El **swap de hojas Fix (v0.11.0)** reorganizo la planilla DESPUES de que este documento se
> escribiera: el Plan de Cuentas paso de `R:T` a `L:N` y de `V:W` a `P:Q`, `Registros` tiene un
> corrimiento de +1 fila, se repuntearon 82 formulas, las pestanas quedaron con 8 nombres
> canonicos y 1909 filas de movimientos incrustados se movieron a la hoja "Cuarentena Plan".
>
> **Todo lo que este documento afirme sobre coordenadas concretas hay que verificarlo contra la
> planilla viva antes de usarlo.** Lo que SIGUE VIGENTE y por eso se conserva: el metodo (las tres
> capas del gemelo, el protocolo de actualizacion, las recetas de auditoria con el TSV), las
> limitaciones declaradas y las trampas documentadas.
>
> Los artefactos derivados (`INVENTARIO_CELDAS.md`, `celdas.tsv`) tambien son pre-swap: se
> regeneran corriendo el scanner y despues los generadores de `devtools/`.
>
> Este archivo y sus herramientas se recuperaron el 2026-08-18 desde la rama
> `feat/arnes-fase-2-gemelo-digital`, que no entro al baseline del swap.
